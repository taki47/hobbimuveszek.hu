    ...
    if (a.length > 0) {
        ...
        if (obj_mobile_nav.back_button == 1) {
            if (obj_mobile_nav.parent_id == 0) {
                ...
                document.getElementById("navbar_mobile_back_button").innerHTML = '<a href="javascript:void(0)" class="nav-link" data-id="' + obj_mobile_nav.parent_id + '" data-category-name="' + c + '"><strong><i class="icon-angle-left"></i>' + obj_mobile_nav.name + "</strong></a>"
            }
...
$(document).ready(function () {
    $("#form_login").submit(function (a) {
        ...
        if (b[0].checkValidity() === false) {
            ...
            a.preventDefault();
            var c = b.find("input, checkbox, button");
            var d = b.serializeArray();
            var loginUrl = $("#form_login").attr("action");
            ...
            $.ajax({
                ...
                success: function (f) {
                    ...
                    if (e.result == 1) {
                        ...
                        if (e.result == 0) {
                            document.getElementById("result-login").innerHTML = e.error_msg
                        }
                    }
                }
            })
...
function update_number_spinner(a) {
    var a = a,
...
$(document).on("input", "#input_search", function () {
    ...
    $.ajax({
        type: "POST",
        url: base_url + "/kereses/ajax-search",
        headers: {
            "X-CSRF-TOKEN": $('meta[name="csrf-token"]').attr('content')
        },
        data: a,
        success: function (e) {
            if ( e ) {
                if ( e.length>0 ) {
                    ...
                    html += "</ul>";
                    $("#response_search_results").html(html);
                    ...
                    html = "<ul><li><a href='#'>"+searchNoResultText+"</a></ul></li>";
                    $("#response_search_results").html(html);
                    $("#response_search_results").show()
                }
            }
...
$(document).on("input", "#input_search_mobile", function () {
    var c = $("#search_type_input_mobile").val();
    var b = $(this).val();
    if (b.length < 2) {
    ...
    $.ajax({
        ...
        success: function (e) {
            var d = JSON.parse(e);
            if (d.result == 1) {
                document.getElementById("response_search_results_mobile").innerHTML = d.response;
